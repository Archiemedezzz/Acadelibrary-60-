    ...
    <div x-data="scholarDiscover()" class="max-w-[1600px] mx-auto w-full flex flex-col xl:flex-row gap-8 xl:gap-10">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <div
                ...
                <div class="relative z-10 w-full max-w-[650px]">
                    <div
                        ...
                        <button class="ml-3 text-gray-500 hover:text-black transition-colors shrink-0">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-5 gap-y-10 pb-10">
                ...
                @forelse($books as $book)
                    <!-- Book -->
                    <button type="button"
                        @click.prevent="openBookModal(@js($book->only(['id', 'title', 'author', 'publisher', 'publication_year', 'cover_image', 'stock', 'description'])))"
                    ...
                    @forelse($trendingBooks as $index => $book)
                        ...
                        <div
                            ...
                            <div class="flex-1 flex flex-col justify-center min-w-0">
                                <h4 class="font-bold text-[13px] text-black leading-tight truncate">{{ $book->title }}</h4>
            ...
            <div class="w-full rounded-[12px] border border-black bg-white p-6 shadow-sm">
                ...
                <div class="flex flex-col gap-3">
                    @forelse($recentlyAddedBooks as $book)
                        <!-- Item -->
                        <a href="#"
                            class="border border-gray-200 rounded-[8px] p-3 flex items-center gap-4 hover:border-gray-400 transition-colors shadow-sm bg-[#FAFAFA]">
                            <img src="{{ $book->cover_image ? asset('storage/' . $book->cover_image) : asset('images/books/default.jpg') }}"
                                alt="{{ $book->title }}"
        ...
        <div x-show="showNewFolderModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
            ...
            <div @click.stop class="relative w-full max-w-md overflow-hidden rounded-[14px] bg-white shadow-2xl p-6">
                ...
                <form method="POST" action="{{ route('scholar.folders.store') }}" class="space-y-4">
                    @csrf
                    <div>
    ...
    <script>
        function scholarDiscover() {
            return {
                ...
                folderStoreAction: '{{ route('scholar.folders.addBook') }}',
                openBookModal(book) {
