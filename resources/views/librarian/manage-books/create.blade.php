...
@section('title', 'Add New Book - Acadelibrary')
...
@section('content')
    <div class="max-w-[1600px] mx-auto w-full flex flex-col gap-6">
        ...
        <div
            ...
            <div>
                <p class="text-[#6B7280] text-sm uppercase tracking-[0.2em] mb-2">Add New Book</p>
                <h1 class="text-[28px] font-bold text-[#111827]">Create a new book record</h1>
                <p class="text-[#4B5563] mt-1">Fill book information and save it into the library collection.</p>
            </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-6">
            <div class="bg-white border border-[#D1D5DB] rounded-[10px] p-6">
                <form action="{{ route('librarian.manage-books.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="description" id="description" rows="5"
                            class="w-full border border-[#D1D5DB] rounded-[6px] px-3 py-2 focus:outline-none focus:ring-0 focus:border-[#000000]">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="bg-white border border-[#D1D5DB] rounded-[10px] p-6">
                <div
                    class="flex items-center justify-center rounded-[10px] border border-[#E5E7EB] mb-5 h-[340px] bg-[#F9FAFB] text-[#6B7280] text-sm">
                    <div class="text-center px-4">
                        <p class="font-semibold text-[#374151] mb-2">Book Cover Preview</p>
                        <p class="text-[#6B7280]">Upload a cover image to see it appear here.</p>
                    </div>
                </div>
                <div class="space-y-4">
                    <div>
                        <p class="text-sm uppercase tracking-[0.2em] text-[#6B7280] mb-1">Create new book</p>
                        <p class="text-[16px] font-semibold text-[#111827]">Fill the details to add a new book into library
                            inventory.</p>
                    </div>
                    <div class="grid gap-4">
                        <div class="bg-[#F8FAFC] border border-[#E5E7EB] rounded-[10px] p-4">
                            ...
                            <ul class="list-disc list-inside text-[#4B5563] text-sm space-y-2">
                                <li>Upload clear book cover image.</li>
                                <li>ISBN should be unique for each book.</li>
                                <li>Leave description empty if not available.</li>
                            </ul>
                        </div>
                        <div class="bg-[#F8FAFC] border border-[#E5E7EB] rounded-[10px] p-4">
                            <p class="text-sm uppercase tracking-[0.2em] text-[#6B7280] mb-2">Format</p>
                            <p class="text-[#4B5563] text-sm">Use a full title and author name to make the book easy to
                                search.</p>
                        </div>
                    </div>
                </div>
