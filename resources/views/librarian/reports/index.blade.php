...
@section('title', 'Reports - Acadelibrary')
    ...
    <div class="max-w-[1600px] mx-auto w-full flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-2">
            <div class="bg-white border border-[#D1D5DB] rounded-[8px] p-6 relative">
                <h3 class="font-serif text-[16px] font-bold uppercase tracking-[0.15em] text-[#1F2937] mb-4">Total Canceled
                </h3>
                <div class="font-sans text-[46px] font-medium text-black leading-none mb-3">{{ $totalCanceled }}</div>
                <p class="text-[16px] text-[#4B5563] font-medium">Canceled transactions in this period.</p>
            ...
            <div class="bg-white border border-[#D1D5DB] rounded-[8px] p-6 relative">
                <h3 class="font-serif text-[16px] font-bold uppercase tracking-[0.15em] text-[#1F2937] mb-4">Total Borrowed
                </h3>
                <div class="font-sans text-[46px] font-medium text-black leading-none mb-3">{{ $totalBorrowed }}</div>
                <p class="text-[16px] text-[#4B5563] font-medium">Books currently issued.</p>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-6 h-6 absolute top-6 right-6 text-black opacity-80">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-15L21 7.5m0 0L16.5 12M21 7.5H7.5" />
            ...
            <div class="bg-white border border-[#D1D5DB] rounded-[8px] p-6 relative">
                <h3 class="font-serif text-[16px] font-bold uppercase tracking-[0.15em] text-[#1F2937] mb-4">Total Overdue
                </h3>
                <div class="font-sans text-[46px] font-medium text-black leading-none mb-3">{{ $totalOverdue }}</div>
                <p class="text-[16px] text-[#4B5563] font-medium">Past due date transactions.</p>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-6 h-6 absolute top-6 right-6 text-black opacity-80">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-15L21 7.5m0 0L16.5 12M21 7.5H7.5" />
        ...
        <div class="bg-white border border-[#D1D5DB] rounded-[10px] p-6 mb-12 flex flex-col gap-6">
            <form method="GET" action="{{ route('librarian.reports') }}"
                ...
                <table class="w-full text-left border-collapse min-w-[900px]">
                    <thead>
                        <tr class="border-b border-[#E5E7EB]">
                            ...
                            <th class="py-3 px-2 text-[18px] font-semibold text-[#6B7280]">Book</th>
                    ...
                    <tbody>
                        @forelse($borrowings as $borrowing)
                            <tr class="border-b border-[#F3F4F6] last:border-none hover:bg-[#F9FAFB]/50 transition-colors">
                                <td class="py-3.5 px-2 text-[#4B5563] font-medium text-[16px]">{{ $borrowing->id }}</td>
                                <td class="py-3.5 px-2">
                                    <div class="flex items-center gap-3">
                                        <img src="{{ $borrowing->user->avatar ? asset('storage/' . $borrowing->user->avatar) : asset('images/avatar-placeholder.webp') }}"
                                            class="w-[36px] h-[36px] rounded-full object-cover border border-[#E5E7EB]">
                                        <div class="flex flex-col">
                                            <span
                                                class="text-[16px] text-black font-semibold">{{ $borrowing->user->name }}</span>
                                            <span
                                                class="text-[13px] text-[#6B7280]">{{ $borrowing->user->id_member }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="py-3.5 px-2">
                                    <div class="flex flex-col">
                                        <span
                                            class="text-[#4B5563] text-[16px] font-medium">{{ $borrowing->borrow_date ? $borrowing->borrow_date->format('d M Y') : '-' }}</span>
                                        <span class="text-[#6B7280] text-[13px] mt-0.5">Due:
                                            {{ $borrowing->return_date ? $borrowing->return_date->format('d M Y') : '-' }}</span>
                                    </div>
                                </td>
                                <td class="py-3.5 px-2">
                                    @if ($borrowing->status === 'returned')
                                        <span
                                            class="bg-[#86EFAC] text-[#166534] px-3 py-1 rounded-[4px] font-bold text-[14px]">Returned</span>
                                    @elseif($borrowing->status === 'borrowed')
                                        <span
                                            class="bg-[#FDE047] text-[#854D0E] px-3 py-1 rounded-[4px] font-bold text-[14px]">Borrowed</span>
                                    @elseif($borrowing->status === 'pending')
                                        <span
                                            class="bg-[#D1D5DB] text-[#374151] px-3 py-1 rounded-[4px] font-bold text-[14px]">Pending</span>
                                    @elseif($borrowing->status === 'overdue')
                                        <span
                                            class="bg-[#FCA5A5] text-[#991B1B] px-3 py-1 rounded-[4px] font-bold text-[14px]">Overdue</span>
                                    @elseif($borrowing->status === 'canceled')
                                        <span
                                            class="bg-[#F3C5CA] text-[#9B1D29] px-3 py-1 rounded-[4px] font-bold text-[14px]">Canceled</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="py-8 text-center text-[#6B7280]">No report transactions found.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
