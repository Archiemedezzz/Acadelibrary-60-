...
@section('content')
<div x-data="librarianHome()" class="max-w-[1600px] mx-auto w-full flex flex-col xl:flex-row gap-6 xl:gap-8">
    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
            ...
            <div
                ...
                <div class="flex gap-3">
                    ...
                    <div class="flex flex-col justify-center">
                        ...
                        <p class="text-[14px] text-gray-500 mt-1 truncate">Requested by
                            {{ $request->user->name }}
                        </p>
                        <p class="text-[12px] text-[#EAB308] mt-2 font-semibold">Pending Approval</p>
                    </div>
                </div>
                <div class="w-full h-[5px] bg-gray-100 mt-3.5 rounded-full overflow-hidden">
                    <div class="h-full bg-[#EAB308] w-[50%]"></div>
                </div>
            </div>
            @endforeach
            @else
            <div
                ...
                <p class="font-medium text-gray-700">No pending borrow approvals right now.</p>
            </div>
            @endif
        </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-5 gap-y-8">
                ...
                <button type="button" @click.prevent="openBookModal(@js($book->only(['id', 'title', 'author', 'publisher', 'publication_year', 'cover_image', 'stock', 'description'])))"
                    ...
                    <div
                        ...
                        <img src="{{ $book->cover_image ? asset('storage/' . ltrim($book->cover_image, '/')) : asset('images/books/default.jpg') }}"
                            alt="{{ $book->title }}"
                ...
                @empty
                <div
                    class="col-span-2 md:col-span-3 lg:col-span-4 xl:col-span-5 rounded-[8px] border border-gray-300 bg-white p-8 text-center text-sm text-gray-500 shadow-sm">
                    No wishlist books found for the selected filters.
                </div>
                @endforelse
            </div>
            ...
            <div class="flex flex-col gap-2.5">
                <div
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="font-sans text-[28px] font-medium text-black leading-none">{{ $totalBooksBorrowed }}
                        </p>
                    </div>
                </div>
                ...
                <div
                    ...
                    <p class="text-[14px] font-medium text-gray-500 mb-1">Total Scholars</p>
                    <p class="font-sans text-[28px] font-medium text-black leading-none">{{ $totalScholars }}</p>
                </div>

                <div
                    class="w-full h-[75px] rounded-[4px] border border-gray-300 bg-white flex flex-col justify-center px-4 shadow-sm">
                    <p class="text-[14px] font-medium text-gray-500 mb-1">Pending Approvals</p>
                    <p class="font-sans text-[28px] font-medium text-black leading-none">{{ $pendingBorrowings }}</p>
                </div>
                ...
                <div
                    class="w-full h-[75px] rounded-[4px] border border-gray-300 bg-white flex flex-col justify-center px-4 shadow-sm">
                    <p class="text-[14px] font-medium text-gray-500 mb-1">Overdue Books</p>
                    <p class="font-sans text-[28px] font-medium text-black leading-none">{{ $overdueBooks }}</p>
                </div>
            </div>
        </div>
                ...
                <div
                    ...
                    <div class="flex items-center gap-2 mb-1.5">
                        <span class="w-3 h-3 rounded-full"
                            style="background-color: {{ $borrowing->status === 'returned' ? '#22C55E' : ($borrowing->status === 'borrowed' ? '#3B82F6' : '#EAB308') }}"></span>
                        <span class="text-[16px] font-bold tracking-widest uppercase"
                            style="color: {{ $borrowing->status === 'returned' ? '#22C55E' : ($borrowing->status === 'borrowed' ? '#3B82F6' : '#EAB308') }}">{{ ucfirst($borrowing->status) }}</span>
                    </div>
                    <p class="text-[15px] text-[#272727] font-medium leading-snug">"{{ $borrowing->book->title }}"
                        <span class="text-gray-500 font-normal">( {{ $borrowing->updated_at->diffForHumans() }}
                            )</span>.
                    </p>
                </div>
                @endforeach
...
<script>
    function librarianHome() {
        return {
            ...
            folderStoreRoute: '{{ route("librarian.folders.store") }}',
            folderAddRoute: '{{ route("librarian.folders.addBook") }}',
            getFolderFormAction() {
